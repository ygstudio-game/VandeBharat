...
| **Version** | 1.2 (Baseline + Chief-Architect Review Supplement + Ray Substrate Revision) |
| **Status** | DRAFT — Chief-Architect Review Complete; Ray substrate revision pending stakeholder approval of **D-14 / ADR-017**; For Stakeholder Approval at Decision Gate |
| **Classification** | Confidential — Engineering |
| **Date** | 2026-07-15 (v1.2 Ray substrate revision); 2026-07-14 (Part II supplement); 2026-07-13 (Part I baseline) |
...
## v1.2 Revision Summary (Ray Substrate)

This revision replaces v1.0's hand-rolled `multiprocessing` + Redis Streams compute substrate with **Ray** (actors + tasks) as the Tier-2 orchestration layer, following the reference architecture in `ray_integrated.jpeg`. This is a **mechanism change, not a policy change**: every concurrency decision, GPU-scheduling strategy, buffer-sizing number, and failure-handling rule from v1.0/v1.1 stands unmodified — what changed is *what implements* them. See **Appendix E, D-14** and **Appendix L, ADR-017** for the formal decision record, and Ch. 5.1/Ch. 6/Ch. 11.5/Appendix I/Appendix O for the affected chapters. The durability boundary (NVMe frame store, PostgreSQL, MinIO) is explicitly **unchanged and still load-bearing** — Ray orchestrates compute, not persistence; this is called out repeatedly below because it is the most common misreading of what adopting Ray buys the system.
...
**Decision (superseded from v1.1 — see Appendix E, D-01R):** A **layered Clean Architecture** with the **domain (Event, Inspection, Detection) at the center**, framework/IO details at the edges, and dependencies pointing **inward** (Dependency Inversion). The runtime substrate for Tier 2 (and the GPU-facing slice of Tier 1) is a **Ray cluster**: a single Ray Head Node (scheduler, resource manager, global control state, dashboard) coordinating a pool of **Ray Actors** (long-lived, stateful — one per GPU, one for the Event Manager, one for I/O) and **Ray Tasks** (short-lived, stateless — frame decode, preprocessing, OCR, aggregation, post-processing), exactly as laid out in the reference architecture (`ray_integrated.jpeg`, Layer 2). This replaces the v1.0 "plain processes + hand-rolled queues" substrate; Ray *is* the process/queue/shared-memory model, just no longer hand-built.

- **Why Clean/layered (unchanged):** the domain concepts (Event, Coach, Component, Defect, Inspection) are stable and safety-critical; the volatile parts (camera SDK, model runtime, DB driver, web framework, **and now the Ray runtime itself**) change often. Clean Architecture keeps the volatile parts out of the core so a camera-SDK swap, a model-runtime swap, or a Ray-version upgrade does not touch domain logic. This directly serves NFR-M1 — `ModelRunner`/`FrameStore`/`Aggregator` ports (5.3) are unchanged; only their *infrastructure* implementation now wraps a Ray actor or task instead of a raw `multiprocessing.Process`.
- **Why Ray, not hand-rolled processes+queues:** the v1.0 baseline's own decision log flagged three problems that Ray solves natively rather than requiring bespoke engineering:
  1. **D-06 (no pixels through pickling queues)** — v1.0 called for "shared memory / Arrow Plasma-style" handoff as a custom build. **Ray's distributed object store *is* Plasma** (Ray is built on it). Frames landed via `ray.put()` are zero-copy-shared across every actor/task on the node (and, for a future multi-node cluster, transferred once and cached) — this appendix's D-06 requirement is met by using Ray correctly, not by building a shared-memory layer from scratch.
  2. **GPU arbitration (11.5)** — v1.0 required a single hand-written scheduler thread to avoid two processes issuing CUDA kernels without coordination. Ray's own scheduler does this: a GPU-bound actor declared with `num_gpus=1` is guaranteed exclusive access to that GPU by Ray's resource accounting; no custom arbiter thread is needed to prevent contention (a priority *policy* on top is still needed — Appendix O — but the mutual-exclusion mechanism itself is Ray's, not ours).
  3. **Fault isolation + restart** — a Ray actor that crashes (OOM, CUDA error, poison input) is detected and can be transparently restarted by Ray (`max_restarts`) without hand-rolled process-supervisor logic per worker type (6.6 still applies at the *cluster* level — supervising the Ray head/workers themselves — but per-worker restart is now a Ray primitive).
- **What does NOT change:** Ray orchestrates **compute**, not **durability**. The Tier-1/Tier-2 seam (Ch. 4), the NVMe durable frame store, and PostgreSQL/MinIO as the systems of record are unchanged and load-bearing exactly as before — a Ray actor's in-memory state (and the object store) is not survivable across a full cluster restart the way a database row is. This distinction is easy to blur and is treated as a hard rule in 6.5 and Appendix J.
- **Alternatives:**
  ...
  - *Full microservices + message bus (Kafka/RabbitMQ) from day one* — rejected as premature for a single-site, single-track v1; the message-bus pattern is what Ray's task queue gives you *internally*, so the migration to a broker-backed multi-site fleet (18.2) is an adapter swap, not a rewrite.
  - *Plain `multiprocessing` processes + Redis Streams (the original v1.0 baseline)* — superseded. It worked, but every one of D-06 (shared memory), GPU arbitration, and process supervision was bespoke engineering duplicating what Ray provides as a maintained primitive. Kept only as the *fallback reference* if a future audit finds Ray's per-node overhead unacceptable on the target edge hardware (OQ-1-adjacent — Ray's own memory footprint must be included in the 6.5/7.9 budget).
  - *Ray Serve for the dashboard API* — considered for Layer 6 (Dashboard); not adopted in v1 because the dashboard is a conventional low-throughput CRUD/query API (Ch. 12) with no batching/GPU need — FastAPI directly is simpler and this is exactly the kind of component Ray's own docs recommend *not* wrapping in an actor. Revisit only if the dashboard itself needs to serve models directly (18.2).
...
| `ModelRunner` | ML | Run a named model on a batch, return detections | `load`, `infer(batch) -> detections`, `unload`, `warmup` — backed in v1 by a Ray GPU Worker actor (11.3, 11.5) |
...
│   ├── ray_actors/             # GPU Worker actors (num_gpus=1), Event Manager actor,
│   │                           # I/O/Data Worker actor — ModelRunner impl wraps these
│   ├── ray_tasks/              # stateless @ray.remote tasks: frame decode, preprocess,
│   │                           # OCR, post-processing, aggregation (Appendix I)
│   ├── ocr/                    # OCR runner (invoked as a Ray task, not its own process)
│   ├── gap/                    # gap detector runner
│   ├── persistence/            # PostgreSQL repositories, migrations
│   ├── object_store/           # MinIO/S3/filesystem gateway
│   ├── messaging/              # queue abstractions — Ray's task queue for in-cluster
│   │                           # dispatch; Redis Streams/NATS/Kafka only where durable,
│   │                           # cross-restart delivery is required (Appendix J.4)
│   └── observability/          # metrics, logging, tracing — plus Ray Dashboard export
├── interfaces/                 # entrypoints
│   ├── api/                    # FastAPI app (dashboard/integration)
│   ├── workers/                # Ray actor/task entrypoints (capture, inference, ...)
│   └── cli/                    # ops CLI (replay, resegment, backfill, `ray status`)
...
# Chapter 6 — Ray Actor/Task Architecture (supersedes "Thread & Process Architecture")

*v1.0 titled this chapter "Thread & Process Architecture" and hand-designed a process/thread map. v1.1 keeps every concurrency **decision** below (the GIL reasoning is exactly as valid as it was) but the **mechanism** is now a Ray cluster: what was "spawn a process, wire a queue, write a supervisor" becomes "declare a Ray actor or task with a resource request." Ray's own scheduler places the work; we specify constraints, not machinery.*

## 6.1 The GIL reality (unchanged — this is why Ray's actor/task split maps cleanly onto it)

Python has a **Global Interpreter Lock**: only one thread executes Python bytecode at a time. This has decisive consequences that a naïve "one thread per stage" picture ignores, and it is *exactly* why Ray's two primitives (actors and tasks) are the right fit rather than an arbitrary choice:

- **I/O-bound work releases the GIL** (blocking C calls: camera SDK reads, socket/DMA, disk writes, `cv2`/NumPy heavy ops in C). This work stays as **threads inside the Edge Acquisition layer** (`ray_integrated.jpeg` Layer 1 — Frame Acquisition Service), *outside* the Ray cluster proper, feeding it via the shared-memory ring buffer.
- **CPU-bound Python work does not scale on threads** and must run as **Ray Tasks** — stateless, short-lived, scheduled by Ray onto whichever CPU worker has a free slot (preprocessing, metadata/aggregation tasks, post-processing — Layer 2's "Ray CPU Worker(s)" box).
- **GPU inference** must own a CUDA context in **dedicated processes** — this is precisely a **Ray Actor** with `num_gpus=1` (Layer 2's "Ray GPU Worker 1..N" boxes): Ray pins that actor to one physical GPU for its lifetime, and no other actor/task can be scheduled onto the same GPU underneath it.

### ⚠ CHALLENGE — "Thread Architecture" as originally stated is still a trap, for the same reason
The original problem statement's "OCR thread, gap thread, preprocessing threads, inference queue" framing implies a threaded design; **a purely threaded design still will not meet the throughput requirement**, Ray or not — preprocessing and inference are CPU/GPU-bound and the GIL serializes them regardless of what schedules the threads. Ray does not repeal the GIL; it gives you a clean, managed way to run **processes** (actors) and **process-pool-dispatched tasks**, which is what the throughput requirement always needed. This remains decision **D-05**, now realized via Ray's actor/task model instead of hand-rolled `multiprocessing`.

## 6.2 Ray cluster topology (replaces the process/thread map)

```mermaid
flowchart TB
    subgraph EDGE["EDGE ACQUISITION (outside the Ray cluster — Layer 1)"]
        direction TB
        TC["12x Camera Threads\n(SDK read, GIL released in C)"]
        FAS["Frame Acquisition Service\n(decompress / format-convert / validate)"]
        RB["Shared Memory / Ring Buffer\n(zero-copy frame buffer)"]
        TC --> FAS --> RB
    end

    subgraph RAY["RAY CLUSTER (Layer 2 — orchestration)"]
        direction TB
        HEAD["Ray Head Node\nTask Scheduler | Resource Manager | Global State | Dashboard"]
        subgraph GPUW["Ray GPU Worker actors — one per physical GPU (num_gpus=1)"]
            G1["GPU Worker 1\nObject Store + GPU Tasks\n(resident model set, Ch. 11.5)"]
            G2["GPU Worker 2"]
            GN["GPU Worker N"]
        end
        subgraph CPUW["Ray CPU Worker(s) — stateless tasks"]
            PRE["Preprocessing tasks"]
            META["Metadata / Aggregation tasks"]
            VAL["Data Validation tasks"]
        end
        subgraph IOW["Ray I/O / Data Worker actor(s)"]
            DW["Data Writer"]
            MC["MinIO Client"]
            DBW["DB Writer"]
            LOG["Log Manager"]
        end
        subgraph EM["Event Manager actor (detached, singleton)"]
    ...
    subgraph TQ["Ray Task Queue (Head Node's internal scheduling queue)"]
        Q1["Frame Decode Tasks"]
        Q2["Inference Tasks"]
        Q3["OCR Tasks"]
        Q4["Post-Processing Tasks"]
        Q5["Aggregation Tasks"]
    end
    ...
    IOW --> DBX[("PostgreSQL")]
    IOW --> OBJX[("MinIO")]
    TQ -.dispatched by.-> HEAD
```

**Reading this against `ray_integrated.jpeg`:** the diagram's Layer 1 (Edge Acquisition) stays exactly as specified in Ch. 3/4 — Ray does not run on a camera thread; it starts at the ring buffer. Layer 2 (Ray Cluster) is this chapter. Layer 3 (NVIDIA RTX / TensorRT inference) is *what runs inside* a GPU Worker actor (Ch. 11). Layers 4–6 (Storage, Training, Deployment) are Ch. 9/10, a new training-pipeline note (11.9), and Ch. 16 respectively.

## 6.3 Per-stage concurrency decisions (Ray primitive replaces process/thread column)

| Stage | v1.0 unit | **Ray primitive (v1.1)** | Count (v1 guidance) | Why | Risk if wrong |
|---|---|---|---|---|---|
| Camera capture | thread | **thread (unchanged — outside Ray, Layer 1)** | 12 (1/cam) | I/O bound, SDK releases GIL; per-camera isolation | one blocking camera stalling others → mitigated by per-thread + bounded ring |
| Ring→NVMe spill | thread | **thread (unchanged — outside Ray, Layer 1)** | 12 | sequential disk writes, I/O bound | disk too slow → back-pressure (Ch. 7) |
| Gap detection | process | **Ray Actor** (small, low-latency; can share a GPU Worker or run its own light actor) | 1 | needs steady latency; isolate from capture GIL; still the only model touching Tier 1 (11.2) | co-locating in capture proc would risk frame drops |
| Preprocessing | process pool | **Ray Task** (`@ray.remote`, stateless) | Ray auto-schedules across CPU Worker slots (= physical cores − reserved) | CPU bound; must bypass GIL; Ray dispatches dynamically instead of a fixed pool | too few CPU Worker slots configured → Tier-2 backlog (tune `num_cpus` per task, not a fixed process count) |
| OCR | process pool | **Ray Task**, low-priority resource class (Appendix O.6) | Ray-scheduled, 2–4 concurrent typical | async, low priority; may share GPU or run CPU | over-provisioning steals GPU from inference (guarded by priority resource class, not a hand-written arbiter) |
| Event Manager | process (single leader) | **Ray detached Actor (singleton)** | **1** | authoritative state, avoids split-brain on Event IDs; a *detached* actor survives its creator and is named, so any component can reach it via `ray.get_actor()` | HA needs leader election later (18.2); v1 single with fast restart from DB, now via Ray's `max_restarts` on the actor |
| Frame assignment | process | **Ray Task** | 1–N, Ray-scheduled | I/O + light CPU; parallel per Event | none major |
| GPU scheduler | thread (custom) | **Ray's own scheduler + a thin priority-policy actor (Appendix O)** | Ray: 1 per cluster (built-in); policy actor: 1 | Ray's placement + resource accounting *is* the single arbiter for GPU mutual exclusion; the policy actor only adds priority ordering on top | a second priority-policy actor → same contention risk as v1.0's "two schedulers" failure mode |
| Inference worker | process | **Ray Actor**, `num_gpus=1` | K per GPU = **one actor per physical GPU** | owns CUDA context; Ray guarantees exclusive GPU access per actor; parallelism via batching + CUDA streams *within* the actor | requesting `num_gpus=1` for more actors than physical GPUs exist → actors queue forever (Ray will not oversubscribe by default) |
| Aggregation | process pool | **Ray Task** | Ray-scheduled | CPU bound dedup/merge | backlog if CPU Worker resources under-provisioned |
| Storage | process pool | **Ray Actor** (I/O/Data Worker — DB + MinIO clients are stateful connections, better held by an actor than re-opened per task) | N actors, or 1 actor with internal connection pool | I/O bound (DB+objects); an actor amortizes connection setup across many writes | too few → write backlog; too many → DB connection-pool exhaustion (9.1) |
| Dashboard API | async worker | **unchanged — plain FastAPI/Uvicorn, outside Ray** (5.1 alternatives) | 2–4 | I/O bound serving, no GPU/batching need | none major |

## 6.4 Producer–Consumer Model (now Ray's task queue + object store, not hand-rolled queues)

The *policy* is unchanged from v1.0 — every seam is still a bounded producer–consumer with an explicit overflow policy (Ch. 4.4.1) — but the *mechanism* for Tier-2/Ray-internal handoff is Ray's own task queue and distributed object store, not an application-level queue class:

```
Producer task/actor --ray.put(frame)--> [ Ray Object Store ] --ray.get()--> Consumer task/actor
                                              │
                                              ├─ object store full? → spill to disk (Ray's own
                                              │   spill-to-disk, tunable) — analogous to, but not
                                              │   a replacement for, the durable NVMe frame store
                                              └─ task queue backs up? → Ray's scheduler naturally
                                                 back-pressures by not admitting new tasks past
...
flowchart LR
    subgraph Producers
        P1["Frame Decode Task"]:::p
        P2["Preprocessing Task"]:::p
    end
    OS{{"Ray Object Store\n(Plasma — zero-copy shared memory)"}}
    subgraph Consumers
        C1["GPU Worker Actor"]:::c
        C2["Aggregation Task"]:::c
    end
    P1 -->|ray.put| OS
    P2 -->|ray.put| OS
    OS -->|ray.get| C1
...
### 6.4.1 Back-pressure propagation (critical for correctness — unchanged policy)
- **Tier 1 (before durability, still outside Ray):** ring buffers use **drop-oldest + loss counter + log + metric** exactly as in v1.0 (NFR-R1/NFR-F4) — Ray plays no role here; this stays in the Edge Acquisition layer.
- **Tier 2 (after durability, now inside the Ray cluster):** back-pressure comes from **not admitting more Ray tasks than there are free resource slots** (`num_cpus`/`num_gpus` requests) — functionally the same "block, don't drop" behavior as v1.0's queues, but enforced by Ray's resource accounting instead of an application-level bounded-queue class.

### 6.4.2 Why bounded, not unbounded (unchanged, now expressed as a Ray resource limit)
An unbounded backlog of submitted Ray tasks still turns a transient slowdown into unbounded memory growth in the object store → OOM → crash → lost train. The equivalent hard rule under Ray: **never submit tasks faster than the cluster's declared resources can drain**, monitored via the Ray Dashboard's queue-depth view (Ch. 15.2) exactly as the old queue-depth metric was.

## 6.5 State ownership & the object store (replaces "Thread-safety & shared state")

- **Event Manager state** is owned by exactly **one detached, named Ray actor** (no shared-memory races on Event creation/IDs) — this is the same single-writer invariant as v1.0's single process, now expressed as Ray's actor-isolation guarantee (an actor processes one method call at a time by default).
- **Ring buffers** (Layer 1, pre-Ray) are unchanged: single-producer / single-or-multi-consumer, lock-free or lock-minimal, producer never blocks (drop-oldest).
- **GPU mutual exclusion is now Ray's resource accounting, not a hand-written scheduler thread.** An actor declared `num_gpus=1` is the *only* thing Ray will schedule onto that GPU. A thin priority-policy actor (Appendix O) still orders *which* work reaches an idle GPU Worker first (inference ≫ OCR, per FR-10) — but the underlying mutual-exclusion mechanism that prevents two processes issuing CUDA kernels into the same context is now Ray's, not ours to maintain.
- **D-06 is now satisfied by Ray's object store directly, not a custom build.** v1.0 required "shared memory / Arrow Plasma-style" handoff to avoid pickling multi-MB frames through queues. **Ray's distributed object store *is* Plasma** — `ray.put()`/`ray.get()` on a frame reference is zero-copy on the local node. The rule stands (no raw pixels through a pickling channel) but the mechanism satisfying it ships with Ray; nothing bespoke to build or maintain here anymore.

## 6.6 Startup, shutdown, supervision (Ray cluster lifecycle)
- **Cluster-level supervision:** the Ray Head Node process itself (and each Ray worker-node process) is still supervised by systemd/a process manager/container orchestration (Ch. 16) — Ray does not supervise *itself*; something must keep `ray start --head` (and worker nodes) alive.
- **Actor-level restart:** each stateful actor (GPU Worker, Event Manager, I/O Worker) is declared with `max_restarts` — Ray detects the crash (OOM, CUDA error, unhandled exception) and restarts the actor automatically, without a hand-written per-worker-type supervisor. This directly replaces the bespoke restart logic v1.0's supervisor needed per process type.
- **Graceful shutdown:** on SIGTERM, capture disarms (Layer 1, unchanged), in-flight Events are finalized/persisted (Event Manager actor drains its queue before `ray.shutdown()`), GPU Worker actors release CUDA contexts on teardown.
- **Crash recovery:** on restart, actors rebuild state from PostgreSQL + the durable frame store exactly as v1.0 specified (Event-ID keyed, idempotent) — **this durability boundary is unchanged by adopting Ray**; Ray's object store and actor state do not survive a full cluster restart, which is precisely why PostgreSQL/MinIO remain the systems of record (5.1).
...
**Recommendation:** design the scheduler to support **A/B/C interchangeably via config**, size the first deployment for **Strategy A or B on a 24–48 GB GPU** (e.g., a single data-center GPU), and treat **D (multi-GPU)** as the scale path (Ch. 18). The choice is data-driven once real model footprints are known (**OQ-4**, decision **D-04**). Strategies A–D are unchanged by the move to Ray — they are a VRAM-residency policy, not a scheduling mechanism, and remain exactly as valid.

### The scheduler, reframed as Ray GPU Worker actors + a thin priority-policy actor (supersedes "single-arbiter scheduler")

**What Ray does for free:** each physical GPU is owned by exactly one **Ray Actor** created with `num_gpus=1` (`ray_integrated.jpeg` Layer 2's "Ray GPU Worker 1..N"). Ray's own resource scheduler guarantees no second actor/task is ever placed on that GPU underneath it — this *is* the mutual-exclusion property v1.0's hand-written "single arbiter" existed to provide. That custom thread is no longer needed for correctness.

**What Ray does *not* do for you, and still needs a policy on top:** Ray has no built-in concept of *priority* between two ready tasks contending for the same actor, and no built-in per-model VRAM ledger. Both remain necessary and are implemented as:
- A **priority-policy actor** (lightweight, one per cluster) that inference and OCR tasks submit through rather than calling a GPU Worker actor directly — it holds the same ordering rule as before (inference ≫ OCR ≫ background anomaly re-scan; OCR cannot preempt inference, enforcing FR-10's spirit) by simply choosing *which already-queued Ray task* to release toward a GPU Worker next. This is a thin queue-ordering shim, not a scheduler — Ray still does the actual placement.
- **VRAM residency (Strategy A/B/C) lives inside each GPU Worker actor's own state** (which models it currently holds loaded) — the actor tracks its own ledger and decides to load/unload on a Strategy-B/C cache-miss; there is no longer a cluster-wide VRAM-ledger process, because a GPU Worker actor already *is* the unit that owns one GPU's VRAM.
- **Batching (11.6)** happens inside the GPU Worker actor's `infer(batch)` method — the actor accumulates requests up to `max_batch`/`max_wait_ms` before calling the model, same policy as before.
- **CUDA streams** overlap H2D/compute/D2H exactly as v1.0 specified, now inside the actor's process (an actor *is* a dedicated process with its own CUDA context — this was true in v1.0 too, it's just Ray-managed now).
- **FIFO-per-train with priority override** is expressed as the priority-policy actor's admission order, unchanged in spirit from v1.0.

```mermaid
flowchart TB
    FA["Frame Assigner (Ray Task)"] --> PRIO["Priority-Policy Actor\n(inference ≫ OCR ≫ background;\nadmission order only —\nnot a GPU scheduler)"]
    OCRQ["OCR Tasks (low prio)"] --> PRIO
    PRIO -->|dispatch| RAYSCHED["Ray Head Node scheduler\n(actual GPU placement —\nnum_gpus=1 mutual exclusion)"]
    RAYSCHED --> W1["GPU Worker Actor 1\n(own VRAM ledger, batch former, CUDA streams)"]
    RAYSCHED --> W2["GPU Worker Actor 2 (optional)"]
    W1 --> AGG["Aggregation (Ray Task)"]
    W2 --> AGG
```

**Net effect:** the *policy* (priority order, VRAM strategy, batching, per-train fairness) from v1.0 is entirely preserved — none of that engineering was wasted. What disappears is the *mechanism* v1.0 had to hand-build to enforce single-GPU-owner mutual exclusion; that mechanism is now a one-line resource request (`num_gpus=1`) on an actor decorator.
...
## 11.9 Training pipeline (new in v1.1 — `ray_integrated.jpeg` Layers 5–6)

The training loop was previously only gestured at (18.3, "Cloud MLOps"). Adopting Ray as the compute substrate makes the training pipeline a natural extension of the same technology rather than a separate system, so it is specified here as a first-class part of the architecture.

**Decision:** training runs on a **cloud GPU cluster** (deliberately *not* the edge portal — training is not latency-bound and must not compete with live inspection for the edge GPU, 18.3's edge-autonomy rule), using **Ray Data** for distributed dataset preprocessing and **MLflow** for experiment tracking and model registry.

```mermaid
flowchart LR
    MINIO[("MinIO\n(Original Frames, Cropped Images,\nFailed Images, Training Dataset)")] --> ING["Data Ingestion"]
    ING --> RDP["Ray Data Pipeline\n- parallel preprocessing (Ray Tasks)\n- augmentation\n- label verification"]
    RDP --> DVER["Dataset Versioning\n(version control, data lineage, reproducibility)"]
    DVER --> CGC["Cloud GPU Cluster (Ray cluster, training)\n- distributed training (Ray Train)\n- hyperparameter tuning (Ray Tune)\n- checkpointing"]
    CGC --> MLF["MLflow Model Registry\n- model tracking, experiments\n- versioning, metrics & artifacts"]
    MLF --> BEST["Best Model selection\n(gated by Ch. 17 acceptance thresholds —\naccuracy is never traded for speed here either)"]
    BEST --> TRTOPT["TensorRT Optimization\n(FP16 / INT8, engine build per target GPU — 11.4)"]
    TRTOPT --> REG[("MODEL_VERSION registry\n(11.4/11.8) — deployed to edge\nvia config change, no code change")]
```

- **Why Ray Data/Train/Tune specifically (and not a bespoke training harness):** the same actor/task primitives and object-store data movement used for inference (6.2–6.5) apply directly to training — a `Ray Data` pipeline distributes preprocessing/augmentation across the cloud cluster's CPU workers exactly as the edge cluster distributes frame preprocessing; `Ray Train` distributes the actual training job across GPU workers with the same `num_gpus`-per-actor pinning discipline as Ch. 11.5. One mental model, two deployments (edge inference cluster, cloud training cluster) — not two different technologies to operate.
- **Why MLflow, not a bespoke registry:** `MODEL_VERSION` (9.3) is still the artifact of record the edge reads from — MLflow is the *upstream* system that produces a promoted, versioned artifact, which is then registered into `MODEL_VERSION` (with its ONNX/TensorRT lineage, 11.4) as the handoff point between the two systems. This keeps the edge's model-loading path (11.4) completely unaware of MLflow's existence — it only ever reads `MODEL_VERSION` + object store, preserving the Ports/Adapters boundary (5.3).
- **Data governance (unchanged from 18.3):** what leaves the OT segment for cloud training, over what encrypted path, per site policy — this pipeline is the *concrete mechanism* 18.3 previously described only as "the growing labeled corpus feeds cloud training pipelines."
- **Failure mode:** a failed/interrupted cloud training run never touches the edge — the edge keeps serving the last promoted `MODEL_VERSION` until a new one passes Ch. 17 acceptance and is explicitly promoted. Training-cluster failures are cost/schedule risk, never an inspection-availability risk (this is the same edge-autonomy principle as 18.3, now made explicit for training specifically).
...
|---|---|---|---|
| **Higher train frequency** | trains arrive faster than Tier 2 clears | more GPU Worker actors / GPUs; Ray autoscaler adds worker nodes | Yes — Ray actors are horizontally scalable, per-train FIFO via the Priority-Policy Actor |
...
| **Multiple portals / fleet** | network of sites | central control plane, cross-site correlation; **multi-node Ray cluster spanning portals, or one Ray cluster per portal + a fleet-level control plane** | Roadmap (K8s + broker + cloud) |
| **Distributed processing beyond one node** | single-node GPU ceiling | **this is now a config change, not an architecture change** — Ray already supports multi-node clusters natively; add worker nodes to the same Ray cluster and GPU Worker actors schedule across them | **Already provisioned — Ray is the v1 substrate (Ch. 6), not a v2 item** (superseded from v1.0, which listed this as a roadmap item before Ray was adopted) |
| **Cloud integration** | central analytics/DR | S3 replication, cloud dashboard, cloud (re)training pipeline (11.9, also Ray-based) | Roadmap (S3 API already used; training pipeline now specified in 11.9) |

## 18.2 v2 substrate evolution (revised — Ray is now v1; this section is genuinely next-horizon)

*v1.0 listed "adopt Ray" here as the headline v2 item. Since Ray is now the v1 substrate (Ch. 6), that line item is done; what remains is the set of things Ray alone does not give you for free — multi-tenancy across sites, durable cross-restart eventing, and orchestrator-level fleet management.*

- **Messaging**: Ray's task queue is in-cluster and does not survive a cluster restart or span sites. For cross-portal, durable, restart-survivable eventing → **Redis Streams / NATS / Kafka**, still an adapter swap behind the `messaging` port (5.4, 8.9) — this item is unchanged from v1.0, just re-scoped to be about *cross-site* durability rather than in-process concurrency (which Ray now owns).
- **Compute (fleet scale)**: a single Ray cluster already spans multiple nodes (18.1) — the genuine v2 step is **KubeRay** (Ray's Kubernetes operator) for declarative, autoscaled, multi-cluster Ray deployment across portals, rather than manually managed `ray start` nodes per site.
- **Orchestration**: Docker Compose → **Kubernetes**, with **KubeRay** managing the Ray cluster lifecycle as a Kubernetes-native resource (16.5) — this is now one coherent step (K8s + KubeRay together) rather than two separate technologies to integrate.
- **Serving surface**: if the dashboard API (Ch. 12) ever needs to serve a model directly (not just query stored results), **Ray Serve** is the natural extension — deliberately not adopted in v1 (5.1) because no such need exists yet.
- **Event Manager HA**: single leader → **leader election** (etcd/Raft), or a Ray-native equivalent using a fault-tolerant detached actor with checkpointed state — removing the SPOF (8.9). This item is unchanged in substance from v1.0.
...
| **D-05** | Concurrency model | threads / **processes+threads** / free-threaded 3.13 | **processes for CPU/GPU, threads for I/O** — realized in v1.1 as Ray tasks (stateless) + Ray actors (stateful/GPU), Ch. 6 | GIL throughput (R-11) | P1 — **closed** by D-14 |
| **D-06** | Frame IPC | pickled queue / **shared memory handles** | **shared memory** — satisfied natively by Ray's object store (Plasma) in v1.1, no custom build (Ch. 6.5) | multi-MB frames can't go through pickle | P1 — **closed** by D-14, no bespoke implementation required |
...
| **D-14** *(new, v1.1)* | Compute orchestration substrate | hand-rolled `multiprocessing` + Redis Streams (v1.0 baseline) / **Ray (actors + tasks)** / Kubernetes Jobs only | **Ray** — Ch. 5.1, 6, 11.5; closes D-05/D-06 as a maintained primitive instead of bespoke code | Determines who builds/maintains GPU mutual exclusion, crash restart, and zero-copy frame IPC — team must budget Ray operational literacy (cluster ops, `ray status`, actor debugging) as a new skill, traded against removing three categories of bespoke, safety-adjacent code | P0 — **this decision is the subject of this v1.1 revision; approval closes the open item from v1.0's Ch. 5.1/18.2** |
...
# Appendix I — Ray Actor & Task Architecture (Complete Responsibility Matrix)

*Extends Ch. 6. Ch. 6 gives the Ray cluster topology; this appendix gives the exhaustive per-worker responsibility, ownership, failure-domain, and restart contract so no worker is ambiguous to implementers. Renamed from "Thread & Worker Architecture" (v1.0) — the Edge Acquisition row(s) below are the only true OS threads left; everything in Tier 2 is now a Ray actor or task.*

## I.1 Master worker responsibility matrix

| Worker | **Ray primitive** | Count | Tier | Owns (exclusive state) | Reads | Writes | Restart contract | Failure domain |
|---|---|---|---|---|---|---|---|---|
| Arm/Presence Controller | thread (Layer 1, outside Ray) | 1 | T1 | ARM/DISARM debounce state | GPIO 2oo2, encoder | ARM/DISARM events, `TRAIN_RUN` create | restart→re-read GPIO; missed arm = fail-safe capture | isolated; loss stops new-train detection only |
| Camera Capture | thread (Layer 1, outside Ray) | 12 | T1 | one camera SDK handle | camera stream, PTP, trigger_seq | ring buffer[cam] | per-thread restart; peers unaffected | one camera |
| Ring→NVMe Spiller | thread (Layer 1, outside Ray) | 12 | T1 | write cursor per cam | ring buffer[cam] | NVMe frames + index rows | restart→resume from last flushed offset | one camera's spill |
| Gap-feed | thread (Layer 1, outside Ray) | 1 | T1 | Cam12 tap cursor | ring buffer[12] | Ray object store (frame ref) | restart→resume from cursor | segmentation input |
| Gap Detector | **Ray Actor** (small, low-latency) | 1 | T1/seam | model context | frame refs (object store) | boundary events | Ray `max_restarts`; encoder cross-check covers gap | segmentation |
| Event Manager | **Ray detached Actor (named singleton)** | 1 | seam | **authoritative Event state + ID sequence** | boundaries, OCR, encoder | `EVENT`, `EVENT_STATE_LOG` | Ray `max_restarts`→rebuild from DB (idempotent); SPOF (R-8) unless HA (18.2) | segmentation/lifecycle |
| Preprocessing | **Ray Task** (stateless) | Ray-scheduled (≈ cores−reserved) | seam/T2 | none (stateless) | NVMe frames | processed tensors → Ray object store | task re-dispatched automatically on failure (stateless) | throughput only |
| OCR | **Ray Task**, low-priority resource class | Ray-scheduled, 2–4 typical | T2 async | none | Cam12 ROIs | identity metadata → Event Manager actor | task retried; never blocks | metadata only (never blocks) |
| Frame Assigner | **Ray Task** | 1–N, Ray-scheduled | T2 | none | `EVENT` bounds, frame index | `FRAME_REF`, FrameSets | stateless retry | per-Event assignment |
| Priority-Policy Actor | **Ray Actor** (thin, admission-order only) | **1** | T2 | admission queue order (not VRAM) | per-model task requests | dispatch signal to GPU Worker actors | Ray `max_restarts`→rebuild order from pending tasks | admission fairness only (GPU mutual exclusion is Ray's, not this actor's) |
| GPU Worker | **Ray Actor**, `num_gpus=1` | K = one per physical GPU | T2 | one CUDA context + resident model(s) + own VRAM ledger | tensor batches (object store) | detections | Ray `max_restarts`→fresh CUDA context, batch re-run (idempotent) | one GPU worker |
| Aggregator | **Ray Task** | Ray-scheduled | T2 | none | detections | aggregated results | stateless retry | per-Event aggregation |
| I/O / Data Worker | **Ray Actor** (holds DB + MinIO connections) | N (or 1 with pooled connections) | T2 | connection pool state | results, images | MinIO, then PostgreSQL (atomic) | Ray `max_restarts`; idempotent upsert by Event ID | persistence |
| Reaper | Ray Task, scheduled periodically (or a timer inside the Event Manager actor) | 1 logical | T2 | none | `EVENT` where state≠terminal | forced transitions + logs | re-scheduled by Ray on failure | stuck-event recovery |
| Retention/Archive | Ray Task (cron-triggered) | 1 logical | bg | none | age/prefix policy | lifecycle transitions, WORM-lock | idempotent, resumable | archive only |
| Dashboard API | async workers, **outside Ray** (5.1) | 2–4 | serve | none | DB, cache, object store | audit rows | stateless, load-balanced | serving |
| Health/Monitoring | sidecar + **Ray Dashboard** (built-in) | 1 (+ Ray's own) | x-cut | none | all metrics, Ray cluster state | Prometheus/logs | restart | observability only |

## I.2 Ray-primitive rationale per worker (replaces the GIL thread-vs-process decision tree)

```mermaid
flowchart LR
    Q{"Is the worker's hot loop\nCPU-bound Python?"}
    Q -->|"No — blocks in C/IO\n(capture, spill — Layer 1, pre-Ray)"| T["THREAD\n(outside the Ray cluster entirely)"]
    Q -->|"Yes, stateless\n(preproc, aggregate, OCR)"| K["RAY TASK\n(@ray.remote, Ray dispatches dynamically)"]
    Q -->|"Owns a CUDA context\n(inference)"| G["RAY ACTOR, num_gpus=1\n(fault-isolated, one context, Ray-pinned)"]
    Q -->|"Authoritative shared state\n(Event Manager, priority policy)"| S["RAY DETACHED ACTOR\n(named singleton, no shared-memory races)"]
```

This is the concrete application of the (renumbered, unchanged-in-substance) GIL decision from Ch. 6.1; every worker in I.1 maps to exactly one leaf. The leftmost leaf (plain OS thread) only applies to Layer 1, which sits outside the Ray cluster by design (6.2).

## I.3 Ownership & concurrency invariants (normative — mechanism updated, guarantees unchanged)

1. **Exactly one** Priority-Policy Actor and **exactly one** Event Manager actor exist at any instant — enforced by Ray's named/detached actor semantics (`ray.get_actor(name)` fails to create a duplicate under the same name), replacing the v1.0 DB-advisory-lock/file-lock lease mechanism.
2. **No image pixels traverse a pickling queue.** Frames move through **Ray's object store** (`ray.put`/`ray.get`, zero-copy via Plasma on-node) — this *is* the mechanism the old rule asked for; nothing bespoke to maintain.
3. **Object store references are managed by Ray's own reference counting**; Ray automatically evicts an object once no task/actor holds a reference to it. This replaces the v1.0 hand-rolled shared-memory-segment refcounting and TTL-based reclaim — Ray owns this lifecycle now.
4. **Every actor exposes liveness via Ray's actor health check** (`ray.util.state` / the Ray Dashboard); `readiness` (can-accept-work) remains an application-level signal each actor reports (e.g., a GPU Worker reports "loading model" vs "ready") consumed by the health view (Ch. 15.5).
...
### ⚠ Read this before J.1 — what Ray changes here and what it does not

Every queue below is still a real design constraint — the **capacities, overflow policies, and priority rules did not change** when Ray became the substrate (Ch. 6). What changed is *which of them are a physical queue you build* versus *an emergent property of Ray's scheduler*:

- **Q1/Q1L (ring buffers)** are unchanged — Layer 1, outside Ray, exactly as specified.
- **Q2–Q4, Q6–Q8 (post-durability Tier-2 queues)** are now **logical** capacities enforced as Ray resource limits (bounded concurrent tasks/actor request queues) rather than a hand-built bounded-queue data structure. The *numbers* in J.1 (capacity, back-pressure trip) remain the correct target values to configure Ray's concurrency limits to — they did not stop mattering, they moved from "a queue you implement" to "a `max_concurrency` / resource request you configure."
- **Q5 (OCR, low priority)** maps to the low-priority resource class in the Priority-Policy Actor (11.5/Appendix O), not a separate physical queue.
- **The DLQ is the one exception that must remain a genuinely durable, external structure** (disk-backed or a durable broker) — Ray's object store and task queue **do not survive a full cluster restart**, so a poison item quarantined only in Ray state would be lost if the cluster restarts. The DLQ is explicitly kept outside Ray for this reason.
...
**Status:** Accepted (endorsed). **Runtime substrate updated by ADR-017** (Ray actors/tasks, not raw `multiprocessing` processes) — the ports-and-adapters/domain-centric decision below is unaffected; only the infrastructure-layer implementation of "cooperating processes" changed. **Relates:** NFR-M1, §5.
**Context:** Single-site, latency-bounded; six teams building in parallel; volatile edges (camera SDK, model runtime, DB, web, **and the Ray runtime**) vs stable safety-critical domain.
**Decision:** Domain at center, dependencies inward, ports as contracts; runtime = cooperating processes (v1.1: Ray actors/tasks), not microservices.
**Consequences:** (+) parallel team development against fakes; SDK/model/DB/Ray swaps don't touch domain; fault isolation without distributed-systems tax. (−) in-process eventing must be migrated to a broker for durable cross-site delivery (provisioned via `messaging` port, 18.2).
**Alternatives rejected:** asyncio single-process monolith (GIL, single crash loses all); day-one microservices+Kafka (premature, hurts latency budget); plain `multiprocessing` + hand-rolled everything (superseded — see ADR-017, this is what v1.0 originally chose over Ray and later revised).

### ADR-004 — Processes for CPU/GPU stages, threads for I/O stages
**Status:** Accepted — **mechanism superseded by ADR-017** (Ray actors/tasks realize this same decision; the reasoning below is still the *why*). **Relates:** D-05, R-11.
...
**Status:** Accepted — **mechanism superseded by ADR-017** (Ray's Plasma-backed object store *is* this, no custom build). **Relates:** D-06.
...
**Status:** Accepted, capacity TBD by OQ-4 — **mutual-exclusion mechanism superseded by ADR-017** (Ray's `num_gpus=1` placement is now the arbiter; residency strategy A/B/C/D and priority ordering remain custom, §O.6). **Relates:** D-04, §O.
...
### ADR-017 — Ray as the Tier-2 compute/orchestration substrate (new, v1.1)
**Status:** Accepted. **Relates:** D-14, §5.1, §6, §11.5, §O. **Supersedes (mechanism only, not intent):** ADR-004 §"dedicated processes for CUDA-context owners", ADR-006 (shared-memory frame IPC), ADR-011 (single-arbiter GPU scheduler).
**Context:** ADR-004/006/011 each required this team to hand-build a piece of general-purpose distributed-systems infrastructure (process-per-CUDA-context supervision, a Plasma-style shared-memory layer, a single-arbiter GPU scheduler) to satisfy requirements that a mature actor/task runtime provides as a maintained primitive. The reference architecture (`ray_integrated.jpeg`) specifies Ray for exactly this role.
**Decision:** Adopt **Ray** as the Tier-2 (and GPU-facing Tier-1, i.e. Gap Detection) compute substrate: stateful/GPU work as **Ray Actors** (`num_gpus=1` for exclusive GPU ownership; a detached named actor for the Event Manager), stateless work as **Ray Tasks** (preprocessing, aggregation, OCR, post-processing), frame handoff via **Ray's object store** (Plasma-backed, zero-copy on-node).
**Consequences:**
(+) D-06 (frame IPC) and the GPU mutual-exclusion half of D-04/ADR-011 are satisfied by a maintained, tested primitive instead of bespoke code the team must debug under production pressure. (+) Actor crash-restart (`max_restarts`) replaces a category of hand-written per-worker-type supervisor logic (ADR-004's fault-isolation intent, cheaper to implement). (+) The Ray Dashboard gives cluster-level observability (task/actor state, resource utilization) for free, extending Ch. 15. (+) Multi-node scaling (18.1's "distributed processing" line item) is now a *configuration* change (add a worker node to the cluster), not an architecture change.
(−) A new operational dependency: the team must operate a Ray cluster (head node lifecycle, `ray status`, actor debugging, object-store memory tuning) — this is a real skill investment, not free. (−) Ray's object store and task queue do **not** survive a full cluster restart — the durability boundary (NVMe frame store, PostgreSQL, MinIO) remains exactly as load-bearing as in v1.0; adopting Ray must not be read as "Ray handles durability too" (a common and dangerous misreading — called out explicitly in §5.1 and §6.6). (−) Admission priority (inference ≫ OCR) and VRAM-residency strategy (A/B/C/D) are *not* provided by Ray and remain fully custom (§O.6) — Ray narrows the engineering surface, it does not eliminate GPU scheduling as a problem.
**Alternatives rejected:** staying on hand-rolled `multiprocessing` + Redis Streams (the ADR-004/006/011 baseline) — rejected because it required this team to build and indefinitely maintain infrastructure-layer code (shared memory, process supervision, GPU arbitration) that is not the team's core competency and is exactly the kind of general-purpose distributed-systems problem a maintained runtime solves better and more safely. Kubernetes Jobs alone (no actor/task runtime) — rejected because K8s schedules containers, not fine-grained in-process actors/tasks; it does not give you Ray's object store or GPU-actor abstraction, and would still require building the equivalent of Ray's task/actor layer on top of it (Ray on Kubernetes, via KubeRay, is the *combination* recommended for the v2 fleet step, 18.2 — not an either/or with Ray itself).
...
*Extends Ch. 11.5–11.6. Adds the scheduling algorithm, batch-formation policy, prioritization, VRAM management, and crash-during-batch semantics (F-10). Updated for the Ray substrate (Ch. 6, ADR-011R) — the mutual-exclusion guarantee formerly labeled "single arbiter" is now Ray's resource scheduler; what remains custom is documented explicitly in O.6.*

## O.1 Scheduler responsibilities, split between Ray and a thin policy actor

```mermaid
flowchart TB
    subgraph RAYCORE["Ray Head Node scheduler (built-in — mutual exclusion mechanism)"]
        RPLACE["Resource-aware placement\n(num_gpus=1 ⇒ exclusive GPU ownership per actor)"]
    end
    subgraph POLICY["Priority-Policy Actor (custom, thin — admission order only)"]
        ADM["Admission\n(per-train FIFO, priority classes)"]
    end
    subgraph GPUACTOR["GPU Worker Actor (custom logic, runs inside the actor)"]
        BATCH["Batch Former\n(per-model, geometry-homogeneous)"]
        VRAM["VRAM Ledger\n(residency A/B/C, load/unload — local to this actor)"]
        STREAM["CUDA Stream Mgr\n(H2D / compute / D2H overlap)"]
        WD["Watchdog\n(per-batch latency, CUDA health)"]
    end
    Qmodels["Per-model inference tasks (Q6 semantics, Appendix J)"] --> ADM
    OCRq["OCR tasks (Q5 semantics, lowest prio)"] --> ADM
    ADM -->|dispatch| RPLACE
    RPLACE -->|exclusive placement| BATCH
    BATCH --> VRAM --> STREAM
    STREAM --> WD
    WD --> WORKERS["(this actor IS the inference worker)"]
    WORKERS --> AGG["Aggregation (Ray Task, Q7 semantics)"]
```

**What moved where, explicitly:** the v1.0 "single arbiter" bundled four responsibilities into one hand-written thread. In v1.1, mutual exclusion (the actual hard part — guaranteeing two processes never contend for one GPU) is Ray's `num_gpus=1` placement, ships tested and maintained. The remaining three responsibilities (admission order, VRAM residency, batch forming/streams) are still custom code — they always required domain-specific policy Ray has no opinion on — but they now live as ordinary Python running *inside* a Ray Actor rather than inside a hand-rolled scheduler process with its own IPC.

## O.2 Scheduling algorithm (per scheduling tick — same steps, different owner per step)

1. **Admit** highest-priority ready work: `priority_train` head-of-line, else oldest train (FIFO); within a train, round-robin Events (fairness, §J.4). *(Priority-Policy Actor, custom.)*
2. **Dispatch to a GPU Worker actor** — this is where Ray's scheduler takes over: it places the dispatched task onto whichever `num_gpus=1` actor is free, guaranteeing no contention. *(Ray, built-in.)*
3. **Form batches** per model up to `max_batch` or `max_wait_ms` (dynamic batching, Ch. 11.6), inside the GPU Worker actor. Tier-2 works on *complete* landed frame sets ⇒ batches are large/full (major efficiency vs online serving). *(GPU Worker actor, custom.)*
4. **Check VRAM ledger** (local to the actor): if the target model is resident (Strategy A) → submit; if not (B/C) → unload coldest model, load target (cached engine, no rebuild), submit. Never exceed VRAM headroom (OOM guard). *(GPU Worker actor, custom.)*
5. **Submit on a CUDA stream**, overlapping H2D copy of batch N+1 with compute of N and D2H of N−1. *(GPU Worker actor, custom.)*
6. **OCR** is admitted only when no inference batch is ready (strict lower priority) — enforced by the Priority-Policy Actor's admission order, not by Ray (Ray has no priority concept between two ready tasks — this is exactly the gap the policy actor exists to fill). Enforces FR-10's spirit on the GPU.
7. **Watchdog** times each batch; on overrun or CUDA error → §P.3 GPU-failure path. On unrecoverable actor failure, Ray's `max_restarts` fires before the old supervisor-restart logic would have needed to.
...
|---|---|---|
| Worker crash mid-batch | Ray detects actor death (no heartbeat) | whole batch re-queued (idempotent, handles still valid on NVMe); Ray restarts the actor with a fresh CUDA context (`max_restarts`); retry (§J.3) |
| CUDA OOM | allocation error inside the actor | reduce `max_batch` for that model, retry; if persistent, drop to Strategy C for the train; alert |
...
| Watchdog timeout (hang) | per-batch latency > `batch_watchdog_ms`, checked by the actor's own watchdog | Ray kills+restarts the actor, batch re-queued, count toward GPU-fault trend |

**Invariant:** no batch failure loses data (frames durable) or double-writes (Event-ID idempotent upsert). Worst case is *latency*, absorbed by SLA slack or surfaced as `processing_incomplete` (Ch. 8.8). This invariant does not depend on Ray — it depends on frame durability (Ch. 7) and Event-ID idempotency (Ch. 9.7), both unchanged by the substrate.

## O.6 What Ray provides vs. what remains custom engineering (summary)

| Responsibility | v1.0 (hand-built) | v1.1 (Ray) | Still custom? |
|---|---|---|---|
| GPU mutual exclusion (no two processes on one GPU) | hand-written single-arbiter thread | `num_gpus=1` on a Ray Actor | **No** — Ray-native |
| Actor/worker crash detection + restart | supervisor process per worker type | Ray `max_restarts` on the actor | **No** — Ray-native |
| Zero-copy frame handoff (D-06) | custom shared-memory / Plasma-style build | Ray's object store (built on Plasma) | **No** — Ray-native |
| Admission priority (inference ≫ OCR) | scheduler's admission logic | Priority-Policy Actor's admission logic | **Yes** — Ray has no task-priority primitive |
| VRAM residency strategy (A/B/C/D) | scheduler's VRAM ledger | GPU Worker actor's local VRAM ledger | **Yes** — domain-specific policy, no generic equivalent |
| Batch formation, CUDA stream overlap | scheduler's batch former | GPU Worker actor's batch former | **Yes** — domain-specific, always would have been custom regardless of substrate |
| Cluster/head-node process supervision | systemd/process manager | still systemd/process manager (Ch. 16) — Ray does not supervise itself | **Yes** — unchanged from v1.0 |

**Takeaway for reviewers:** adopting Ray did not eliminate GPU scheduling as an engineering problem — Strategies A–D, batching, and priority ordering are still real work this team must build and tune. What it eliminated is the *lowest-level, highest-risk-to-get-wrong* part (safe concurrent GPU access and crash recovery), which is exactly the part a general-purpose distributed runtime is best positioned to get right once and maintain, versus reinventing it per project.
