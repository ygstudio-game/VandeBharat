...
## Corpus Check
- 74 files · ~241,556 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 211 nodes · 237 edges · 19 communities (12 shown, 7 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 7 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `978097bb`
...
3. `run_sync()` - 7 edges
4. `scripts` - 7 edges
5. `generate_report()` - 6 edges
6. `correlate_coach()` - 6 edges
7. `_PDF` - 5 edges
8. `detect_segments()` - 5 edges
9. `Running Everything (Development)` - 4 edges
10. `build_pdf_report()` - 4 edges
...
Cohesion: 0.28
Nodes (6): _all_cameras_done(), ExtractRequest, get_conn(), Frame Extractor Service — port 5003 Receives video path → OpenCV extracts every, True when every session_camera for this session has frame_count > 0., run_extraction()
...
Cohesion: 0.15
Nodes (12): BaseModel, CorrelateRequest, OcrRequest, generate(), GenerateRequest, get_conn(), get_report(), Report Generator Service — port 5006 POST /generate { session_id } → builds PDF (+4 more)
...
## Knowledge Gaps
- **66 isolated node(s):** `fastify`, `cors`, `multipart`, `config`, `prisma` (+61 more)
